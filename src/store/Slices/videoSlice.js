...
const videoSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getAllVideos.fulfilled, (state, action) => {
            ...
            // If the response is an array, we directly assign it to state.videos.docs
            if (Array.isArray(action.payload)) {
                state.videos.docs = [...state.videos.docs, ...action.payload];
                state.videos.hasNextPage = action.payload.length > 0; // Set based on whether there are items
            } else {
